...
async fn test_get_air_quality_validation_latitude_high() {
    ...
    let result = service.get_air_quality(90.001, 11.6, None, None).await;
...
async fn test_get_air_quality_validation_longitude_high() {
    ...
    let result = service.get_air_quality(48.1, 180.001, None, None).await;
...
fn test_air_quality_request_validation_empty_hourly_daily() {
    ...
    assert!(
        req.validate().is_ok(),
        "AirQuality with coordinates should be valid"
    );
}
